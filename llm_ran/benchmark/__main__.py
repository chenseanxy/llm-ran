...
parser.add_argument(
    "--chain",
    type=str,
    choices=["kubernetes_direct_chain", "kubernetes_codegen_chain"],
    required=True,
...
if args.chain == "kubernetes_direct_chain":
    graph = kubernetes_direct_chain(model=model)
elif args.chain == "kubernetes_codegen_chain":
...
# tests
# poetry run python -m llm_ran.benchmark --trials 1 --model qwen2.5:32b --scenarios base --prefix test --pull-model --chain kubernetes_direct_chain